...
from app.api.schemas.cnpj_request import CNPJRequest
...
@router.post("/analyze", response_model=CNPJResponse)
async def analyze_cnpj(request: Request, payload: CNPJRequest):
    check_rate_limit(request, limit=20, window=60)
    
    try:
        response = await CNPJEnrichmentService.analyze(payload.cnpj)
        return response
